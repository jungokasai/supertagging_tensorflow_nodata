...
#            assert len(sequences[i]) == len(suffix_sequences[i])
        ...
        data = data[indices]
        cap_indicator = cap_indicator[indices]
        num_indicator = num_indicator[indices]
        suffix_data = suffix_data[indices]
        labels = labels[indices]
        if opts.jackknife:
            jk_labels = jk_labels[indices]
        #labels_nums = labels_nums[indices]
        ...
        if opts.jackknife:
            self.jk_labels = jk_labels[:-nb_validation_samples]
        self.train_cap_indicator = cap_indicator[:-nb_validation_samples]
        self.train_num_indicator = num_indicator[:-nb_validation_samples]
        self.suffix_train = suffix_data[:-nb_validation_samples]
        ...
        #self.y_test_nums = labels_nums[-nb_validation_samples:]

        nb_train_samples = data.shape[0] - nb_validation_samples
        if opts.jackknife:
            K = 10 
	...
	#    if not new_opts.embedding_dim == 300: # word2vec is of 300 dim
        ...
        if new_opts.jackknife:
            import pickle
            with open('../k_fold/predictions.pkl', 'rb') as fhand:
                self.jackknife_train_mat = pickle.load(fhand)
            with open('../k_fold/predictions_test.pkl', 'rb') as fhand:
